...
describe('user participations sql repository', () => {
    ...
    describe('get all by user tests', () => {
        test('without users inserted', async () => {
            const users_participations = new UserParticipationsSql(PostgresSqlConfig)

            const response = await users_participations.getAllByUser(new Email('test@test.es'))

            expect(response.length).toBe(0)
        })
        describe('with a user inserted', () => {
            let user_data = {
                id: 1,
                email: 'test1@test.es',
                username: 'test1',
            ...
            test('without inscriptions', async () =>{
                ...
                const response = await users_participations.getAllByUser(new Email(user_data.email))
            ...
            describe('with inscriptions', () => {
                ...
                test('with one user inscription', async () => {
                    ...
                    const response = await userparticipations.getAllByUser(new Email(user_data.email))
