...
const SELECT_ESCAPE_ROOMS = 'SELECT * FROM escaperooms, locations, cities, countries WHERE escaperooms.physical_location = locations.id AND locations.city = cities.id AND cities.country = countries.id;'
const SELECT_ESCAPE_ROOM_BY_ID = 'SELECT e.id, e.title, e.description, e.solution, e.difficulty, e.price, l.id as physical_location, l.coordinates, l.street, l.street_number, l.other_info, ci.id as city, ci.city_name, co.id as country, co.country_name FROM escaperooms e JOIN locations l ON e.physical_location = l.id JOIN cities ci ON l.city = ci.id JOIN countries co ON ci.country = co.id WHERE e.id = $1;'
const DELETE_ESCAPE_ROOM_BY_ID = 'DELETE FROM escaperooms WHERE id = $1 RETURNING id;'

export { INSERT_ESCAPE_ROOM, INSERT_LOCATION, INSERT_CITY, INSERT_COUNTRY, SELECT_CITY_BY_NAME, SELECT_COUNTRY_BY_NAME, SELECT_LOCATION, SELECT_ESCAPE_ROOMS,SELECT_ESCAPE_ROOM_BY_ID, DELETE_ESCAPE_ROOM_BY_ID }