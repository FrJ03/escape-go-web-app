...
const SELECT_ESCAPE_ROOMS = 'SELECT e.id, e.title, e.description, e.solution, e.difficulty, e.price, l.id as physical_location, l.coordinates, l.street, l.street_number, l.other_info, ci.id as city, ci.city_name, co.id as country, co.country_name FROM escaperooms e, locations l, cities ci, countries co WHERE e.deleted = FALSE AND e.physical_location = l.id AND l.city = ci.id AND ci.country = co.id;'
